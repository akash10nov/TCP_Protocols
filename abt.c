...
int sequenceID=0;
int waitForAck=0;
struct pkt A_resendpkt;
struct pkt B_resendpkt;
int A=0;
int B=1;
int AorB=0;
int ACKNO=0;
  ...
  struct msg message;
{
	...
	if(waitForAck==1)
	{
		printf("Sender is looking for acknowledgement so it can't send new message\n");
	}
	else{
		
		struct pkt send_packet;
		strncpy(send_packet.payload,message.data,20);
		send_packet.seqnum=sequenceID;
		waitForAck=1;
	    send_packet.acknum=(ACKNO+1)%2;
		int i=0;
		send_packet.checksum=send_packet.seqnum+send_packet.acknum;
		for(i=0;i<20;i++)
		{
			send_packet.checksum=send_packet.checksum+(int)send_packet.payload[i]; // this will add seqnum to every data we have so on other end we can detect data corruption efficiently since there will 20 bits and there is any alteration you can say that data is corrupted.
		}
		
		printf("sequence number:%d \n",send_packet.seqnum);
		printf("checksum: %d\n",send_packet.checksum);
		A_resendpkt=send_packet;
		waitForAck=1;
		AorB=0;
		tolayer3(A,send_packet); 
		A_transport++;
        starttimer(A,50);// start timer
	}
}
...
int ackfromreceiver=0;
/* called from layer 3, when a packet arrives for layer 4 */
void A_input(packet)
  struct pkt packet;
{
	struct msg receivedack;
	int Check2=packet.seqnum+packet.acknum;
	int i=0;
	for(i=0;i<20;i++)
	{
		Check2=Check2+(int)packet.payload[i];
	}
	printf("Check2: %d\n",Check2);
	printf("A input pack ack num:%d\n",packet.acknum);
	printf("ACK:%d\n",ackfromreceiver);
	if(packet.acknum==ackfromreceiver && packet.checksum==Check2)
	{
		AorB=0;
		printf("ack received \n");
		if(sequenceID==0)
		...
		stoptimer(AorB);
		waitForAck=0;
		ackfromreceiver=(ackfromreceiver+1)%2;
		strncpy(receivedack.data,packet.payload,20);
		tolayer5(AorB,receivedack.data);
	}
}
...
{
    printf("I am resending the packet again due to packet loss.\n");
	AorB=0;
	waitForAck=1;
	//stoptimer(A);
	tolayer3(AorB,A_resendpkt);
	A_transport++;
	starttimer(AorB,50);
}  
...
void B_input(packet)
  struct pkt packet;
{
	int Check_sum=packet.checksum;
	int check1=packet.seqnum+packet.acknum;
	int i=0;
	B_transport++;
	for(i=0;i<20;i++)
	{
		check1=check1+(int)packet.payload[i];
	}
	//printf("checksumafaf: %d\n",check1);
	printf("seq num: %d\n",packet.seqnum);
	...
	if(Check_sum==check1)
	{
		...
		//receivedData.data=packet.payload;// since we only need to send the acknowledgement so sending the data back again doesn't make any sense.
		...
		ackpack.checksum=ackpack.seqnum+ackpack.acknum;
		for(i=0;i<20;i++)
		{
			ackpack.checksum=ackpack.checksum+(int)ackpack.payload[i];
		}
		printf("checksum from B's ack: %d\n",ackpack.checksum);
		B_resendpkt=ackpack;
		tolayer5(AorB,receivedData.data);
		B_application++;
		tolayer3(AorB,ackpack);
		
		printf("packet should have been accepted\n");
	}
